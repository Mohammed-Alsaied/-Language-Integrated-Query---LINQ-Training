...
namespace LINQTut14.StandardMethod
{
    internal class Program
    {
        ...
        private static void RunCount()
        {
            ...
            Console.WriteLine($"Toatal Questions {quiz.Count(q => q.Marks == 1)}");
            Console.WriteLine($"Toatal Questions wuth mark == 1: {quiz.Where(q => q.Marks == 1).Count()}");
            Console.WriteLine($"Toatal Questions using LongCount: {quiz.Where(q => q.Marks == 1).LongCount()}");
        ...
        {
            var quiz = QuestionBank.GetQuestionRange(Enumerable.Range(1, 200));
            var maximumMark = quiz.Where(x => x.Choices.Count < 3).Min(x => x.Marks);
            Console.WriteLine(maximumMark);
        }
        ...
        {
            var quiz = QuestionBank.GetQuestionRange(Enumerable.Range(1, 200));
            var maximumQuestionMark = quiz.MinBy(x => x.Marks);
            Console.WriteLine(maximumQuestionMark);
        }
        ...
        {
            var quiz = QuestionBank.GetQuestionRange(Enumerable.Range(1, 200));
            var MarkSum = quiz.Sum(x => x.Marks);
